...
Channiwala-Parikh ultimate-analysis correlation:
...
Stoichiometric air is calculated from complete-combustion oxygen demand:
...
residual `O2` as balancing species.

## Pollutants and PCDD/F

Sulfur is split mainly between `H2S` and `SO2`. Chlorine is split mainly between
`HCl` and `Cl2`. Fuel nitrogen is split coarsely among `NH3, HCN, NO, NO2, N2O,
N2`.

Dioxins/furans are reported as qualitative risk plus broad screening ranges,
not as validated concentration predictions.
...
The char output is a screening composition, not a substitute for measured char
proximate/ultimate analysis or GC-MS.
